...
internal class AfterDarkDomainResolver {
    ...
    @Volatile
    private var classicAttempted = false

    /**
     * Background-safe phase. It performs at most one regular HTTP attempt and
     * never opens UI. Playback waits for this phase when it is still running.
     */
    suspend fun resolveInBackground(): String? {
    ...
    /**
     * Playback phase. Reuse the result of the background HTTP attempt, or wait
     * for it if it is still running. The interactive WebView is allowed only
     * here, after the regular HTTP path has failed.
     *
     * cachedOrigin is deliberately process-only: it is never written to disk.
     */
    suspend fun resolveForPlayback(): String {
        cachedOrigin?.let { return it }
        resolveInBackground()?.let { return it }

        return resolutionMutex.withLock {
            cachedOrigin?.let { return@withLock it }

            Log.i(TAG, "Ouverture du registre AfterDark dans le WebView interactif")
            val webViewOrigin = AfterDarkDomainWebView.resolve()
                ?: throw ErrorLoadingException(
                    "Impossible de récupérer l'adresse actuelle d'AfterDark depuis le WebView",
                )

            cachedOrigin = webViewOrigin
            Log.i(TAG, "Domaine AfterDark obtenu par WebView : $webViewOrigin")
            webViewOrigin
        }
    }
